  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200/80 hover:border-forest-200 transition-all shadow-xs overflow-hidden flex flex-col justify-between">
      ...
      <div className="p-4 space-y-4 flex-1">
        ...
        {/* Destination & Point d'arrivée si configuré */}
        {(order.destination_city_snapshot || order.destination?.city_name || order.depot_name_snapshot || order.depot?.name) && (
          <div className="p-2.5 rounded-xl bg-forest-50/50 border border-forest-100/80 text-xs space-y-1">
            <div className="flex items-center justify-between text-[11px] font-semibold text-forest-900">
              <span className="flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-forest-600" />
                Arrivée : {order.destination_city_snapshot || order.destination?.city_name || order.delivery_city}
              </span>
              {(order.expected_arrival_date_snapshot || order.destination?.expected_arrival_date) && (
                <span className="text-[11px] text-forest-700 font-bold flex items-center gap-1">
                  <Calendar className="w-3 h-3 text-forest-500" />
                  {new Intl.DateTimeFormat("fr-FR", { day: "numeric", month: "short", year: "numeric" }).format(new Date(order.expected_arrival_date_snapshot || order.destination!.expected_arrival_date))}
                </span>
              )}
            </div>
